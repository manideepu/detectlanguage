...
import com.language.exception.NGramException;
...
public class GramTreeBuilder extends AbstractGramTree {
	 ...
	 *
	 * @param text
	 *            character sequence to learn n-grams from.
	 */
	 ...
	 *
	 * @param gram
	 *            n-gram to be added to the tree.
	 */
	...
	/**
	 * A node of the n-gram tree. Consists of a character, its frequency, and
	...
	private static class NodeBuilder extends AbstractNode implements Comparable<NodeBuilder> {
		...
		private void inc() {
			if (freq == Integer.MAX_VALUE) {
				throw new NGramException(
						"Maximum frequency is reached. N-Gram is too frequent in the corpus. Try to use a smaller corpus.");
			}
			freq++;
		}
		...
		/**
		 * Builds an immutable n-gram tree from this builder data. The minimal
		 * frequency value is used to discard n-grams which are supposed to be
		 * irrelevant for a language, reducing the total amount of memory
		 * required to model a language.
		 * 
		 * @param minFreq
		 *            the minimum number of occurrences an n-gram must have been
		 *            seen to be considered relevant
		 * @return an immutable n-gram tree
		 */
		private AbstractNode build(final int minFreq) {
			if (childcount == 0)
				return new MinimalNode(c, freq, null);
			List<AbstractNode> children2 = new LinkedList<AbstractNode>();
			for (int i = 0; i < childcount; i++) {
				if (children[i].freq >= minFreq)
					children2.add(((NodeBuilder) children[i]).build(minFreq));
			}
			return new MinimalNode(c, freq,
					children2.isEmpty() ? null : children2.toArray(new AbstractNode[children2.size()]));
		}
